...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="http://xfycn.com.cn/css/search_result.css">
</head>
<body>
...
else
{
  ...
  $name = $_POST['Name'];
  $result = mysql_query("SELECT * FROM teacher_information WHERE tname = '".$name."'");
  $inf = array();
  $i = 0;
    ...
    for($j=0; $j<$i; $j=$j+2){
      if($i > $j + 1){
        ...
        <!--偶数的人-->
           ...
           <span><?php echo '<br><br>姓名：<a href="TeacherDetail.php?tid=',$inf[$j + 1][0],'">',$inf[$j + 1][1],'</a>' ?></span>
           <span>
           <?php 
           if($inf[$j + 1][2] == 0){
            echo '<br><br>性别：男';
      ...
      //有奇数人对应这个名字
        ...
        <table  border="0" align="left">
           <img src="<?php echo "http://xfycn.com.cn/pic/".$inf[$j][0].".png";?>" width=150px length=150px/>
           <span><?php echo '<br><br>姓名：<a href="TeacherDetail.php?tid=',$inf[$j][0],'">',$inf[$j][1],'</a>' ?></span>
           <span>
           <?php 
           if($inf[$j][2] = 0){
            echo '<br><br>性别：男';
